...
import { getPrintRequests, PrintRequest, getBankAccounts, BankAccount, BANKS, User } from '../../userStore';
...
interface DebtReportPageProps {
  customerZalo: string;
  allUsers: User[];
  allRequests: PrintRequest[];
}

const formatCurrency = (value: number) => new Intl.NumberFormat('vi-VN').format(value) + 'đ';

export const DebtReportPage: React.FC<DebtReportPageProps> = ({ customerZalo, allUsers, allRequests }) => {
    ...
    useEffect(() => {
        // Find customer name from either user list or the first invoice
        const foundUser = allUsers.find(u => u.zalo === customerZalo);
        

        const customerInvoices = allRequests.filter(r => 
            r.orderDetails.customerInfo.zalo === customerZalo &&
    ...
    }, [customerZalo, allUsers, allRequests]);
    ...
    const handleCopyLink = () => {
        navigator.clipboard.writeText(window.location.href).then(() => {
            showToast('Đã sao chép link báo cáo!', 'success');
        });
    };
