...
export const ImageAnalysisFeedback: React.FC<ImageAnalysisFeedbackProps> = ({ result, isLoading }) => {
  if (isLoading) {
    return (
      <div className="mt-4 p-4 bg-slate-100 dark:bg-zinc-800/50 rounded-lg flex items-center">
        <div className="w-4 h-4 border-2 border-zinc-400 dark:border-zinc-500 border-t-zinc-600 dark:border-t-zinc-300 rounded-full animate-spin mr-3"></div>
        <p className="text-sm text-slate-600 dark:text-zinc-300 font-medium">Đang phân tích ảnh của bạn...</p>
      </div>
    );
  }
      ...
      <ul className="space-y-2 text-sm">
        {result.gender && (
            <li className="flex items-start">
                <UserCircleIcon className="w-4 h-4 text-slate-500 dark:text-zinc-400 mr-2.5 mt-0.5 flex-shrink-0" />
        ...
        {result.age && (
            <li className="flex items-start">
                <CalendarIcon className="w-4 h-4 text-slate-500 dark:text-zinc-400 mr-2.5 mt-0.5 flex-shrink-0" />
                <span className="text-slate-700 dark:text-zinc-300">
        ...
        {result.feedback?.map((item, index) => (
          <li key={index} className="flex items-start">
            ...
            <span className={item.isGood ? 'text-slate-600 dark:text-zinc-300' : 'text-slate-700 dark:text-zinc-200'}>{item.message}</span>
          </li>
        ))}
      </ul>
    </div>
