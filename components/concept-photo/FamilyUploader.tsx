...
export const FamilyUploader: React.FC<FamilyUploaderProps> = ({ familyMembers, onFamilyChange, maxMembers }) => {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files) {
      ...
      const newMembers: FamilyMember[] = files.map(file => ({
        ...
        role: 'adult_female', // Default role, will be updated by analysis
        height: '',
        isAnalyzing: true,
      }));
  ...
  const handleMemberUpdate = (id: string, updates: Partial<FamilyMember>) => {
    onFamilyChange(familyMembers.map(m => m.id === id ? {...m, ...updates} : m));
  };
  ...
  return (
    <div className="space-y-4">
      ...
      />
      <label 
          htmlFor="family-uploader-input"
          title={addButtonTooltip}
          className={`w-full p-4 border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-lg flex flex-col items-center justify-center text-center transition-colors ${isAddDisabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:border-purple-500 hover:bg-purple-50 dark:hover:bg-slate-700/50'}`}
      >
          <MaleIcon className="w-6 h-6 text-slate-500 dark:text-slate-400 mb-1" />
          <span className="text-sm text-slate-600 dark:text-slate-300 font-semibold">Tải ảnh thành viên</span>
          <span className="text-xs text-slate-500 dark:text-slate-400 mt-1">Có thể chọn nhiều ảnh cùng lúc</span>
      </label>
              ...
              <div key={member.id} className="bg-slate-100 dark:bg-slate-700/50 p-2 rounded-lg">
                <div className="flex items-start">
                    ...
                    <div className="flex-grow min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <select 
                            value={member.role} 
                            onChange={(e) => handleMemberUpdate(member.id, { role: e.target.value as MemberRole })}
                            className="text-sm font-semibold text-slate-800 dark:text-slate-200 bg-transparent border-none focus:ring-0 p-0"
                        >
                      ...
                      <p className="text-xs text-slate-500 dark:text-slate-400 truncate" title={member.file.name}>{member.file.name}</p>
                       <div className="mt-1 flex items-center gap-2">
                          ...
                          <input
                              ...
                              type="number"
                              value={member.height || ''}
                              onChange={(e) => handleMemberUpdate(member.id, { height: e.target.value })}
                              placeholder="cm"
                              className="w-full max-w-[80px] p-1 text-sm border border-slate-200 dark:border-slate-600 rounded-md bg-white dark:bg-slate-900/50 text-slate-800 dark:text-zinc-200 focus:ring-1 focus:ring-purple-500 focus:border-purple-500"
                          />
                      </div>
                    </div>
                ...
                {visibleAnalysis[member.id] && member.analysisResult && (
                    <div className="mt-2 pt-2 border-t border-slate-200 dark:border-slate-600 text-xs text-slate-600 dark:text-slate-300 space-y-1 pl-1">
                        {member.analysisResult.gender && <p><strong className="font-semibold">Giới tính (AI):</strong> {member.analysisResult.gender}</p>}
                        {member.analysisResult.age && <p><strong className="font-semibold">Tuổi (AI):</strong> {member.analysisResult.age}</p>}
